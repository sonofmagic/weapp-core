...
const escapedLocal = weightedSelectors.map(selector => escapeLocal(selector))
const escaped501 = weightedSelectors.map(selector => escape501(selector))

describe('@weapp-core/escape unescape weighted mix (~5% leading hyphen/digit)', () => {
  bench('unescape weighted selectors (local)', () => {
    for (const selector of escapedLocal) {
      unescapeLocal(selector)
    }
  })

  bench('unescape weighted selectors (5.0.1)', () => {
    for (const selector of escaped501) {
      unescape501(selector)
    }
  })
})
